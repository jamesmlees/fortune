    ...
    describe("filters", function(){
      ...
      describe('update operations filter', function(){
        describe('PATCH', function(){
          ...
          it('should work fine with complex filters for PATCH', function(done){
            request(baseUrl).patch('/people/' + ids.people[1] + '?filter[and][0][or][0][email][regex]=' + ids.people[1])
              .set('content-type', 'application/json')
              .send(JSON.stringify([
                {op: 'replace', path: '/people/0/name', value: 'passed'}
              ]))
              .expect(200)
              .end(function(err, res){
                should.not.exist(err);
                var body = JSON.parse(res.text);
                body.people[0].name.should.equal('passed');
                done();
              });
          });
          it('should send 403 if filter does not match any document', function(done){
            request(baseUrl).patch('/people/' + ids.people[1] + '?filter[and][0][or][0][email][regex]=' + ids.people[0])
              .set('content-type', 'application/json')
              .send(JSON.stringify([
                {op: 'replace', path: '/people/0/name', value: 'passed'}
              ]))
        ...
        describe('DELETE', function(){
              ...
              .end(function(err){
                ...
                request(baseUrl).get('/people')
                  .end(function(err, res){
                    ...
                    body.people.length.should.equal(ids.people.length - 1);
                    done();
                  });
              });
          });
          it('should be able to apply provided filter to individual DELETE request', function(done){
            request(baseUrl).get('/people/' + ids.people[1])
              .end(function(err, res){
                should.not.exist(err);
                var body = JSON.parse(res.text);
                request(baseUrl).del('/people/' + ids.people[0] + '?filter[name]=' + body.people[0].name)
                  .end(function(err){
                    should.not.exist(err);
                    request(baseUrl).get('/people/' + ids.people[0])
                      .end(function(err, res){
          ...
          it('should send 403 if filter does not match any document', function(done){
            request(baseUrl).del('/people/'+ ids.people[0] + '?filter[and][0][or][0][email][regex]=' + ids.people[1])
              .expect(403)
              .end(function(err, res){
                should.not.exist(err);
                request(baseUrl).get('/people/' + ids.people[0])
