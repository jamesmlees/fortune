...
function route(name, model, resources, inflect, querytree) {
  ...
  function getResources(req, res) {
    ...
    beforeReadHook({}, req, res)
      .then(function(){
        ...
        return querytree.parse(model.modelName, _.extend(match, req.query.filter)).then(function(query){
          ...
          var queries = [];
          queries.push(adapter.findMany(model, query, projection));

          if(req.query.includeMeta) {
            queries.push(adapter.count(model, {}, projection));

            if (req.query.filter) {
              queries.push(adapter.count(model, query, projection));
            }
          }

          return RSVP.all(queries);
    ...
    // run after read
      .then(function(resources) {
        return RSVP.all(resources[0].map(function(resource) {
          return afterReadHook(resource, req, res);
        }))
        .then(function(r) {
          var out = { resources : r };
          if (resources.length > 1) out.count = resources[1];
          if (resources.length > 2) out.filterCount = resources[2];
