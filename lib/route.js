...
function route(name, model, resources, inflect, querytree) {
  ...
  function createResource(req, res){
    ...
    if (req.body.linked){
      ...
      _.each(omitExternal(req.body.linked), function(linkedResources, key){
        linkedBefores[key] = runBefores(inflect.singularize(key), linkedResources);
      });
      RSVP.hash({
        mainResources: runBefores(model.modelName, mainResources),
        linkedResources: RSVP.hash(linkedBefores)
      })
        .then(function(transformed){
          //Check if any before hook returned false
          if (hooksFailed(
            _.flatten(_.values(transformed.linkedResources), true).concat(transformed.mainResources)
          )) return;
        ...
        if (_.size(ids) > 0) {
          ...
          findPromise = beforeReadHook(relatedModel, {}, req, res)
            .then(function(){
              ...
              return adapter.findMany(relatedModel, ids, projection.limit ? projection.limit : undefined);
            }, function(err){
              sendError(req, res, 500, err);
            });
        } else {
  ...
  function deleteResources(req,res){
    ...
    adapter.findMany(model)
      ...
      .then(function(){
        return adapter.delete(model, req.query.filter);
      }, function(err){
        sendError(req, res, 500, err);
      })
  ...
  function deleteResource(req, res) {
    ...
    req.query.filter = req.query.filter || {};
    req.query.filter.id = {in: id};

    // find the resource by ID
    adapter.findMany(model, req.query.filter)
  ...
  function updateResource(req, res) {
      ...
      req.body.forEach(function (operation) {
        ...
        if (operation.op === 'replace') {
          if (field.length > 1 && field[0] !== "links") {
            ...
            path[field[field.length-1]] = value;
          }
          ...
          if (field.length === 2){
            ...
            path[pullKey][field[0]].push(field[1]);
          }else{
            throw new Error('Expected /resource/0/:path/:id for remove op, got ' + operation.path);
          }
        }
      });
    } catch(error) {
    ...
    // update the resource
      .then(function(update) {
        req.query.filter = req.query.filter || {};
        req.query.filter.id = id;
        return adapter.update(model, req.query.filter, update);
      }, function(error) {
        sendError(req, res, 403, error);
      })
    ...
    // do after transform
      .then(function(resource) {
        if (!resource) return sendError(req, res, 403);
        return afterWriteHook(resource, req, res);
      }, function(error) {
        sendError(req, res, 500, error);
      })
  ...
  function getLinked(ids, type, req, res) {
    ...
    if (ids.length > 0) {
      ...
      director.methods.get(collection, {
        params: {id: ids.join(',')},
        query: {
