...
Fortune.prototype._exposeResourceDefinitions = function() {
  ...
  this.router.get(resourceRoute, function(req,res){
    if (_this._ready){
      ...
      console.error(_this.options.serviceName, ' is not up yet. Waiting for ', Object.keys(_this._resources).length, ' resources to be configured. Initialised resources count: ', _this._resourcesStarted);
      res.writeHead(503);
      res.end();
    }
  });
...
Fortune.prototype.resource = function(name, schema, options, schemaCallback) {
  ...
  this._resources[name] = { schema: schema, modelOptions: modelOptions, hooksOptions: hooksOptions, policy: policyOptions};
...
Fortune.prototype._preprocessSchema = function (schema) {
  ['id', 'href', 'links', 'in', 'or', 'and', 'then', '__isNew'].forEach(function (reservedKey) {
