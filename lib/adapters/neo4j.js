...
adapter._init = function (options) {
  ...
  this.db = new neo4j.GraphDatabase(options.connectionString);
...
adapter.create = function (model, resource) {
  var _this = this;

  console.log("neo4j create", resource);

  console.log("neo4j _this.db", _this.db);
  
  var node = _this.db.createNode(resource);
  ...
  return new Promise(function (resolve, reject) {
    ...
    node.save(function (err, node) {    // ...this is what actually persists.
        if (err) {
          ...
          console.log('Node saved to database with id:', node.id, node);
          resolve([_.extend(node.data, { id : node.id })])
        }
    });
