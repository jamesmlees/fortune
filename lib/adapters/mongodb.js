...
adapter.update = function (model, filter, update) {
  var _this = this;
  model = typeof model == 'string' ? this.model(model) : model;
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (filter && filter._bsontype === "ObjectID" || !_.isObject(filter)){
      match = {};
      match[pk] = filter;
    }else{
      var match = filter;
    ...
    model.findOneAndUpdate(parseQuery(model, match, pk), update, function(error, resource) {
...
adapter.delete = function (model, query) {
  var _this = this;
  model = typeof model == 'string' ? this.model(model) : model;
  var pk = model.pk || "_id";
  ...
  return new Promise(function(resolve, reject) {
    var match = query || {};
    var parsed = parseQuery(model, match, pk);
    model.find(parsed).exec(function(error,resources){
      model.remove(parsed, function(error){
        if(error){
          reject(error);
        } else {
          resolve(resources);
...
function parseQuery(model, query, pk){
  var dbQuery = {};
  query = _.clone(query);
  ...
  _.each(query, function(val, key){
    ...
    if(_.isNull(val) || _.isUndefined(val)){
      ...
      query['$' + key] = _.map(val, function(subquery){
        return parseQuery(model, subquery, pk);
      });// _.partial(parseQuery, model, _, pk));
      delete query[key];
    }
  });
...
adapter.findMany = function(model, query, projection) {
  ...
  if (_.isObject(query)){
    query = parseQuery(model, query, pk);
...
adapter._dissociate = function (model, resource) {
  ...
  _.each(model.schema.tree, function(branch, path){
    if (branch.ref && branch.inverse && !branch.external){
      var values = _.isArray(resource[path]) ? resource[path] : _.isUndefined(resource[path]) ? [] : [resource[path]];
      var relatedModel = adapter.model(branch.ref);
      var upd = {};
      if (_.isArray(relatedModel.schema.tree[path])){
        upd.$pull = {};
        upd.$pull[branch.inverse] = resource.id;
      }else{
        upd.$unset = {};
        upd.$unset[branch.inverse] = true;
      }
      promises.push(values.map(function(id){
        adapter.update(relatedModel, id, upd);
      }));
    }
  });
